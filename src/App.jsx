...
/* ─── VERSION ────────────────────────────────────────────────────────────── */
const APP_VERSION = "v0.6 — Mar 19 2026";
...
const SmartRowText = ({ text, seenAbbr }) => {
  ...
  while ((match = regex.exec(text)) !== null) {
    ...
    if (!seenAbbr.has(raw)) {
      seenAbbr.add(raw);
      // First occurrence — render as link
      parts.push(
        <span key={match.index} style={{position:"relative",display:"inline"}}>
          <a
            ...
            style={{color:T.terra,textDecoration:"underline",textDecorationStyle:"dotted",textUnderlineOffset:3,fontWeight:600,cursor:"pointer"}}
          >
            {match[0]}
          </a>
          {tooltip===raw&&(
            <span style={{position:"absolute",bottom:"100%",left:0,zIndex:99,background:T.ink,color:"#fff",borderRadius:8,padding:"6px 10px",fontSize:11,whiteSpace:"nowrap",boxShadow:"0 4px 16px rgba(0,0,0,.25)",marginBottom:6,pointerEvents:"none"}}>
